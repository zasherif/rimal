...
def run_tests() -> int:
    cases = [
        ...
        TestCase("Boolean Ops", TEST_PROGRAMS / "boolean_ops.rml", "خطأ\nصح\nصح\nصح\n"),
        ...
        TestCase("Shadowing OK", TEST_PROGRAMS / "shadowing_ok.rml", "٢\n١\n"),
        TestCase("Assert OK", TEST_PROGRAMS / "assert_ok.rml", ""),
        ...
        TestCase(
            "Assert Fail",
            TEST_PROGRAMS / "assert_fail.rml",
            None,
            expected_error_substr="unreachable",
        ),
        TestCase(
            "Invalid: Immutable Assign",
            TEST_PROGRAMS / "invalid_immutable_assign.rml",
            None,
            expected_error_substr="Cannot assign to immutable binding",
        ),
        TestCase(
            "Invalid: Undeclared Var",
            TEST_PROGRAMS / "invalid_undeclared_var.rml",
            None,
            expected_error_substr="Use of undeclared variable",
        ),
        TestCase(
            "Invalid: Bool Condition",
            TEST_PROGRAMS / "invalid_bool_condition.rml",
            None,
            expected_error_substr="If condition must be منطقي",
        ),
    ]
